...
export default function SponsorCard({ sponsorData }) {
  ...
  if (sponsorData) {
    sponsorName = (
      <h1 className="text-[20px] font-semibold">{sponsorData.name}</h1>
    );
    sponsorImage = (
      <Image
    ...
    if (sponsorData.instagram) {
      instagramLink = (
        <Instagram color={"#0163AA"} linkTo={sponsorData.instagram} />
      );
    }

    if (sponsorData.facebook) {
      facebookLink = (
        <Facebook color={"#0163AA"} linkTo={sponsorData.facebook} />
      );
    }
